...
create table if not exists chirps (
  ...
  source      text not null default 'heartbeat'
              check (source in ('heartbeat', 'manual', 'system')),
  created_at  timestamptz not null default now()
);

create index if not exists chirps_flock_id_idx on chirps (flock_id);
create index if not exists chirps_created_at_idx on chirps (created_at);
create index if not exists chirps_bird_id_idx on chirps (bird_id);

create table if not exists codes (
  id               text primary key,
  code             text not null unique,
  handle_reserved  text not null,
...
create index if not exists codes_handle_idx on codes (handle_reserved);
create index if not exists codes_ip_created_idx on codes (ip, created_at);
