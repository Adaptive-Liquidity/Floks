...
export function standingOrdersFor(role: string): string {
  ...
  if (lower.includes("chief") || lower.includes("staff")) {
    return "Coordinate the flock. Do not take customer actions without approval.";
  }
  if (lower.includes("sales") || lower.includes("outreach")) {
    return "Draft outreach. Never send mail or name customers in public chirps.";
  }
  ...
  return `Act as ${r}. Coordinate with the flock. Do not take customer actions without approval.`;
}
...
export function buildClonePrompt(input: {
  ...
  return [
    ...
    `Paste the join prompt into your chief of staff so it can follow ${input.origin}/skill.md and publish your flock.`,
    "",
